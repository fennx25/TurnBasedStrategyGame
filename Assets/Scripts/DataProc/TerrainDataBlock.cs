...
namespace DataProc
{
    ...
    public class IHeightToColor : IColorize<Single> //1-1024的浮点数，实际上一般不到50 //Floating point numbers of 1-1024, in fact generally less than 50
    {
    ...
    // 这个接口用于把数据转换成十六进制格式显示在文本框里面，浮点例外，为一位小数
    //This interface is used to convert the data into hexadecimal format and display it in the text box, except for floating point, it is a decimal
    public interface IHexViewable<T>
    {
    ...
    public class TerrainDataBlock<T> : IExportGridInfo where T : struct, IFormattable
    {
        ...
        public void Draw1(UnityEngine.Transform parent, UnityEngine.Transform cube, float y, float width, float height)
        {
            int i, j;
            int nRowsCount = tContent.GetLength(0);
            int nColumnsCount = tContent.GetLength(1);
            ...
            for (i = 0; i < nRowsCount; i++)
            {
                for (j = 0; j < nColumnsCount - 1; j++)
                {
                    ...
                    //aBrush.Dispose();
                    T b = tContent[i, j];
                    Color a = iColorize.PointToColor(b);
                    ...
                    UnityEngine.Transform c = UnityEngine.GameObject.Instantiate(cube);
                    c.parent = parent;
                    c.position = new UnityEngine.Vector3(j * dWidthUnit, y, i * dHeightUnit);
                    c.localScale = new UnityEngine.Vector3(a.R / 255f, a.R / 255f, a.R / 255f);
                }
            }
        }

        public T[] FlattenOpacityBytes()
        {
            ...
            T[] res = new T[nRowsCount * nColumnsCount];
